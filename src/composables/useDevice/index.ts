...
export function useDevice(): Device {
  ...
  const windows = ref<boolean>(false);
  const macOS = ref<boolean>(false);
  const desktop = ref<boolean>(false);

  function update() {
    const userAgent = navigator.userAgent;

    ios.value = isIos(userAgent);
    android.value = isAndroid(userAgent);
    windows.value = isWindows(userAgent);
    macOS.value = isMacOS(userAgent);
    desktop.value = !isMobile(userAgent);
    mobile.value = isMobile(userAgent);
  }
